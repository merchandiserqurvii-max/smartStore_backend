...
var getOrderDetails = asyncHandler(async function (req, res) {
  ...
  var cuttingMaster = '';
  try {
    // step 1: fetch orders records for this order
    ...
    // step 2: filter for cutting master location and sort by scanned_timestamp descending
    var scanTrackingRes = await axios.get(NOCODB_BASE + '/misuaa9cvim4h13/records', {
      params: {
        offset: '0',
        limit: '100',
        where: '(order_id,eq,' + orderId + ')',
    ...
    var scanTrackingList =
      scanTrackingRes.data && scanTrackingRes.data.list ? scanTrackingRes.data.list : [];

    var cmRecords = scanTrackingList.filter(function (r) {
      return (
        r.locations.id === CUTTING_MASTER_LOCATION_ID ||
    ...
    if (cmRecords.length > 0) {
      var rawName =
        cmRecords[0].employees && cmRecords[0].employees.user_name
...
var getRecords = asyncHandler(async function (req, res) {
  ...
  if (order_id) {
    ...
    values.push(String(order_id));
  }
  if (style_number) {
    conditions.push('style_number = $' + idx++);
    values.push(String(style_number));
  }
  if (record_type) {
...
// GET /api/scan/cutting-masters
var getCuttingMasters = asyncHandler(async function (req, res) {
  var pool = getPool();
  var result = await pool.query('SELECT * FROM cutting_masters ORDER BY name ASC');
  res.status(200).json(new ApiResponse(200, result.rows, 'Cutting masters fetched'));
});

// POST /api/scan/cutting-masters  — Admin only
var addCuttingMaster = asyncHandler(async function (req, res) {
  var name = (req.body.name || '').trim();
  var employee_id = (req.body.employee_id || '').trim();
  if (!name) throw new ApiError(400, 'name is required');
  var pool = getPool();
  var result = await pool.query(
    'INSERT INTO cutting_masters (name, employee_id) VALUES ($1, $2) RETURNING *',
    [name, employee_id || null]
  );
  res.status(201).json(new ApiResponse(201, result.rows[0], 'Cutting master added'));
});

// DELETE /api/scan/cutting-masters/:id  — Admin only
var deleteCuttingMaster = asyncHandler(async function (req, res) {
  var pool = getPool();
  var result = await pool.query(
    'DELETE FROM cutting_masters WHERE id = $1 RETURNING *',
    [parseInt(req.params.id, 10)]
  );
  if (!result.rows.length) throw new ApiError(404, 'Cutting master not found');
  res.status(200).json(new ApiResponse(200, null, 'Cutting master deleted'));
});
...
module.exports = {
  ...
  getCuttingMasters: getCuttingMasters,
  addCuttingMaster: addCuttingMaster,
  deleteCuttingMaster: deleteCuttingMaster,
};
